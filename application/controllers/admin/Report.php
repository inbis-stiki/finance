...
class MyPDF extends TCPDF{
    ...
    public function setTableExp($header,$data,$jenExp) {
        $this->setTableConf();
        $this->Ln(10);
        // Header
        if($jenExp == "3"){
...
class Report extends CI_Controller{
    ...
    public function main(){
        if($_POST['jenis'] == '1'){
            ...
            $this->expense($_POST['period'], $_POST['tgl'], $_POST['jenis']);
        }
    }
    ...
    public function expense($filter, $tgl, $jenExp){
        $jenExpense = "";
        if($jenExp == '3'){
            $jenExpense = "BBM";
        }else if($jenExp == '4'){
            ...
            $jenExpense = "Lain - Lain";
        }
        $pdf = new MyPDF('Landscape', PDF_UNIT, "A4", true, 'UTF-8', false);
        $pdf->HeaderTitle = 'Expense ('.$jenExpense.')';
    ...
    public function getDataExp($param){
        $filter = "";
        if($param['filter'] == '1'){ // getByMonth
            $filter = '
                YEAR(t.transaksi_tanggal) = "'.$param['year'].'"
        ...
        return $this->db->query('
            ...
            WHERE
                '.$filter.'
                AND t.id_pengeluaran = mjp.pengeluaran_id
                AND mjp.pengeluaran_jenis  = "'.$param['jenExp'].'"
            GROUP BY 
                t.transaksi_keterangan ,
                t.id_sparepart ,
                t.no_rangka ,
